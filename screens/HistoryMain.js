...
function HistoryMain({navigation}) {
  ...
  const content = [
    {id: 1, key: 'learned', image: require('../src/learned.png'), state: 'learned'},
    {id: 2, key: 'learning', image: require('../src/learning.png'), state: 'learning'},
    {id: 3, key: 'skipped', image: require('../src/skipped.png'), state: 'skipped'},
  ];
    return (
      <SafeAreaView style={styles.container}>
        <FlatList
          ...
          renderItem={({item}) => {
            const count = wordsCount[item.state] || 0;
            const title = t(`history.${item.key}.title`);
            // Switch the subtitle copy when the section is empty — "Выполнены
            // все повторения" under "Выученные · 0" reads as if you already
            // finished something, which is the opposite of what 0 means.
            const description =
              count > 0
                ? t(`history.${item.key}.subtitle`)
                : t(`history.${item.key}.empty`);
            return (
            <TouchableOpacity onPress={() => navigation.push("HistoryScreen", {title, mode: item.state})} style={styles.button}>
